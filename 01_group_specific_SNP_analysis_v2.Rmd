...
getUniqueSnps <- function(vcf.file, samplesDT, qual = 40) {
  ...
  results <- lapply(groups, function(currGroup) {
    ...
    currUniverse <- samplesDT[-grep(paste0("(", currGroup, "|EWT)"), group), samples] # nolint: object_usage_linter.
...
dt2 <- dcast(counts, group ~ chr, value.var = "N", fun.aggregate = sum)
...
datatable(dt2, options = list(pageLength = 25, scrollX = TRUE))
```
...
pps <- lapply(names(groupUniqueSnps), function(currGroup) {
  ...
  combi <- factor(tapply(geno$sub, geno$allele, paste, sep = "-", collapse = "-"), levels = combn)
...
#### Generating intersection of the groups compare the coresponding EWT samples

```{r}
pps <- lapply(samplesDT[!(is.na(meta) | grepl("OTHERS", meta)), unique(meta)], function(currMeta) {
  ##
  geno <- do.call(rbind, lapply(samplesDT[meta == currMeta, unique(group)], function(currGroup) {
    vcf <- groupUniqueSnps[[currGroup]]
    currSamples <- samplesDT[group == currGroup, samples]
    dt <- cbind(
      allele = rownames(geno(vcf)$GT),
  ...
  p_bar <- ggplot(df, aes(combi, Freq)) +
    geom_col() +
    labs(title = paste("Sample", currMeta, "and corresponding EWTs")) +
    theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust = 1))
...
#### Saving the intersection of the EWT comparaison as a table

##### Common SNPs between GBA1 edited cell lines and there corresponding EWTs

```{r}
meta <- samplesDT[!(is.na(meta) | grepl("OTHERS", meta)), unique(meta)]
pps <- lapply(meta, function(currMeta) {
  ##
  geno <- do.call(rbind, lapply(samplesDT[meta == currMeta, unique(group)], function(currGroup) {
    vcf <- groupUniqueSnps[[currGroup]]
    currSamples <- samplesDT[group == currGroup, samples]
...
##### Common SNPs between SNCA-A30P edited cell lines and there corresponding EWTs

```{r}
datatable(pps[["SNCA-A30P"]])
```
...
```{r fig.height=10, fig.width=10, warning=FALSE, eval = FALSE}
...
allGroup <- mclapply(groupUniqueSnps, function(vcf) {
  ...
  subject <- introns[elementNROWS(introns) > 0]
  query <- rowRanges(vcf)
  vtype <- "intron"
  ignore.strand <- FALSE
  asHits <- FALSE
  ...
  ## Reorder the GRanges based on the names of the original VCF
  all[factor(names(all), levels = names(vcf))]
}, mc.cores = detectCores(), mc.preschedule = FALSE)
```
...
rawResult <- lapply(names(allGroup), function(currGroup) {
  ...
  dt <- do.call(rbind, lapply(levels(all$LOCATION), function(feature) {
    ...
    return(unique(dt[, ENTREZID := NULL]))
  }))
  legend <- data.table(
    group = currGroup,
    sample = samples,
  ...
  return(list(legend = legend, dt = dt))
})
...
write.csv(do.call(rbind, lapply(rawResult, "[[", "dt")), file.path("data", "SNP_annotations_update2.csv"))
...
results2 <- mclapply(names(groupUniqueSnps), function(currGroup) {
  ...
  seqlevels(vcf) <- seqlevels(vcf)[seqlevels(vcf) %in% seqlevels(txdb)]
  cds <- unique(locateVariants(vcf, txdb, CodingVariants()))

  aa <- unique(predictCoding(vcf[cds$QUERYID], txdb, Hsapiens))
  aa <- aa[!is.na(aa$GENEID)]
