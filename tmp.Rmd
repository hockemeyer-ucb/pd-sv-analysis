...
```{r}
vcf.file <- grep("chr1\\.", vcf.files, value = TRUE)
...
if (file.exists(resultToSave) && file.exists(vcfToSave)) {
  ...
  rawResults[[grep("chr1\\.", vcf.files)]]

  "chr1_228761069_T_C" %in% rownames(rawResults[[grep("chr1\\.", vcf.files)]][["ATP13A2"]])
  "chr1_228761069_T_C" %in% rownames(rawResults[[grep("chr1\\.", vcf.files)]][["EWT-SNCA-A30P"]])
...
"chr1_228761069_T_C" %in% rownames(groupUniqueSnps[["ATP13A2"]])
"chr1_228761069_T_C" %in% rownames(groupUniqueSnps[["EWT-SNCA-A30P"]])
```
...
sum(duplicated(row.names(vcf)))



f <- !geno(vcf["chr1_228761069_T_C"])$GT %in% c("0/0", "./.", "./0", "0/.")

geno(vcf["chr1_228761069_T_C"])$GT[f]
